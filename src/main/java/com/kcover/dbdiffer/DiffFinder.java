...
public class DiffFinder{
    ...
    public static void writeDiffReportToFile(JdbcTemplate oldDbTemplate, JdbcTemplate newDbTemplate, int pageSize, FileWriter fileWriter) {
        try {
    ...
    public static void writeMissingAccountsToFile(JdbcTemplate oldDbTemplate, JdbcTemplate newDbTemplate, int pageSize, FileWriter fileWriter) {
        //create a function to be performed for each page fetched from the oldDb
        Consumer<List<Map<String, Object>>> pageHandler = resultList -> {
            List<Account> oldAccounts = resultList.stream().map(Account::fromMap)
    ...
    public static void writeCorruptedAccountsToFile(JdbcTemplate oldDbTemplate, JdbcTemplate newDbTemplate, int pageSize, FileWriter fileWriter){
        //create a function to be performed for each page fetched from the oldDb
            Consumer<List<Map<String, Object>>> pageHandler = resultList -> {
                List<Account> oldAccounts = resultList.stream().map(Account::fromMap)
                        .collect(Collectors.toList());
    ...
    public static void writeNewAccountsToFile(JdbcTemplate oldDbTemplate, JdbcTemplate newDbTemplate, int pageSize, FileWriter fileWriter) {
        //create a function to be performed for each page fetched from the newDb
        Consumer<List<Map<String, Object>>> pageHandler = resultList -> {
            List<NewDbAccount> newDbAccounts = resultList.stream().map(NewDbAccount::fromMap)
                    .collect(Collectors.toList());
    ...
    /**
     * Iteratively creates SQL queries that will grab a page of accounts from the DB of the given JdbcTemplate
     * and pass them to a function. Use this to traverse a DB in chunks and perform some function on all the
     * accounts within. Passes the resulting list of maps from the query to the pageHandler. It's then up to the pageHandler
     * to convert the response into something it knows how to use.
     */
    private static void traverseDbAndPerform(JdbcTemplate jdbcTemplate, int pageSize, Consumer<List<Map<String, Object>>> pageHandler){
        if(pageSize < 1){
            throw new IllegalArgumentException("Page size must be greater than 0.");
        }

        long accountsCount = jdbcTemplate.queryForObject("SELECT COUNT(id) FROM accounts", Integer.class);
